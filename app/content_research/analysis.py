...
class DirectionalAnalysisService:
    ...
    async def analyze(self, *, task, direction: dict[str, Any], query: str, facts: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        try:
            ...
            await self._record(request.context, response.provider, response.model, response.usage, response.latency_ms, "success", None)
            ...
            await self._record(request.context, "unknown", "unknown", TokenUsage(), None, "failed", str(exc))
            return None

    async def _record(self, context, provider, model, usage, latency_ms, status, error_message) -> None:
        ...
        async with LLMUsageTracker(self._db_path) as tracker:
            await tracker.record(LLMUsageEventInput(
