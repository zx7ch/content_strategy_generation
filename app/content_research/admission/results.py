...
DIRECTION_RESULT_ALGORITHM_VERSION = "direction_result_v2"
...
def build_direction_result(*, direction_id: str, policy_snapshot_id: str, decisions: list[ClaimAdmissionDecisionRecord]) -> DirectionAdmissionOutput:
    ...
    decision_identity = ",".join(sorted(item.id for item in decisions))
    result_id = "drd_" + hashlib.sha256(
        f"{direction_id}:{policy_snapshot_id}:{decision_identity}".encode()
    ).hexdigest()[:24]
