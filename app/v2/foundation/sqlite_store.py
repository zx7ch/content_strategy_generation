...
class SQLiteMasterDataStore:
    ...
    def _connect(self) -> sqlite3.Connection:
        if self._writer is not None:
            conn = open_readonly_database(self._db_path)
            conn.row_factory = sqlite3.Row
            return conn
        conn = open_bootstrap_database(self._db_path)
        conn.row_factory = sqlite3.Row
        conn.execute("PRAGMA journal_mode=WAL")
        conn.execute("PRAGMA foreign_keys=ON")
        return conn

    async def _submit(self, action: str, fields: list[Any]) -> None:
        if self._writer is None:
            raise RuntimeError("runtime writer is not configured")
        try:
            await self._writer.submit(
                TypedMutation.create(
                    mutation_id=f"master_data_{uuid.uuid4().hex}",
                    mutation_kind="mutate_master_data",
                    domain_payload={"action": action, "fields": fields},
                )
            )
        except DomainMutationRejectedError as exc:
            raise ValueError(exc.safe_message) from None

    def _submit_sync(self, action: str, fields: list[Any]) -> None:
        if self._writer is None:
            raise RuntimeError("runtime writer is not configured")
        try:
    ...
    async def save_workspace_async(self, workspace: WorkspaceRecord) -> WorkspaceRecord:
        if self._writer is None:
        ...
        if self._writer is not None:
            self._submit_sync(
                ...
                [
                    ...
                    _fmt_dt(brand.created_at),
                    _fmt_dt(brand.updated_at),
                ],
            )
            return brand
        with self._connect() as conn:
    ...
    async def save_brand_async(self, brand: BrandRecord) -> BrandRecord:
        if self._writer is None:
        ...
        if self._writer is not None:
            self._submit_sync(
                ...
                [
                    ...
                    _fmt_dt(channel.created_at),
                    _fmt_dt(channel.updated_at),
                ],
            )
            return channel
        with self._connect() as conn:
    ...
    async def save_brand_channel_async(
        self,
        channel: BrandChannelRecord,
    ) -> BrandChannelRecord:
        if self._writer is None:
            return self.save_brand_channel(channel)
        await self._submit(
            "save_brand_channel",
            [
                channel.id,
        ...
        if self._writer is not None:
            self._submit_sync(
                ...
                [
                    ...
                    _fmt_dt(policy.created_at),
                    _fmt_dt(policy.updated_at),
                ],
            )
            return policy
        with self._connect() as conn:
    ...
    async def save_policy_config_async(
        self,
        policy: BrandPolicyConfigRecord,
    ) -> BrandPolicyConfigRecord:
        if self._writer is None:
            return self.save_policy_config(policy)
        await self._submit(
            "save_policy_config",
            [
                policy.id,
        ...
        if self._writer is not None:
            self._submit_sync(
                ...
                [
                    ...
                    _fmt_dt(snapshot.computed_at),
                    _fmt_dt(snapshot.valid_from),
    ...
    async def save_state_snapshot_async(
        self,
        snapshot: BrandStateSnapshotRecord,
    ) -> BrandStateSnapshotRecord:
        if self._writer is None:
            return self.save_state_snapshot(snapshot)
        await self._submit(
            "save_state_snapshot",
            [
                snapshot.id,
