...
def test_selection_is_stable_when_candidate_order_changes():
    groups = compile_query_groups(direction_id="product_marketing", subject="短裤", questions=["卖点"], competitors=[])
    candidates = [
        {"canonical_id": "note_b", "query_group_id": groups[0].id, "relevance": 1, "author_id": "a"},
        {"canonical_id": "note_a", "query_group_id": groups[0].id, "relevance": 1, "author_id": "b"},
    ]
    ...
    assert first == second
    assert [item.canonical_source_id for item in first.decisions if item.selected] == ["note_a", "note_b"]


def test_author_cap_excludes_extra_candidate_without_breaking_selection():
    groups = compile_query_groups(direction_id="product_marketing", subject="短裤", questions=["卖点", "口碑"], competitors=[])
    result = select_candidates(groups=groups, candidates=[
...
def test_packet_hash_changes_only_with_projected_evidence():
    ...
    first = build_packet(direction_id="product_marketing", canonical_source_id="note_1", fields={"content_text": "轻量"}, availability={"content_text": "present"}, retrieval_context=context)
    second = build_packet(direction_id="product_marketing", canonical_source_id="note_1", fields={"content_text": "轻量"}, availability={"content_text": "present"}, retrieval_context=context)
...
def test_time_window_and_unavailable_blocking_fields_are_explicit_exclusions():
    groups = compile_query_groups(direction_id="product_marketing", subject="短裤", questions=["卖点"], competitors=[])
    result = select_candidates(groups=groups, candidates=[
        {"canonical_id": "late", "query_group_id": groups[0].id, "source_published_at": "2026-07-02T00:00:00+00:00"},
        {"canonical_id": "missing", "query_group_id": groups[0].id, "blocking_unavailable": True},
    ], author_cap=1, run_as_of_at=datetime(2026, 7, 1, tzinfo=timezone.utc))
...
def test_packet_excludes_raw_payload_and_tokens_from_projection_and_context():
    packet = build_packet(
        direction_id="product_marketing", canonical_source_id="note_1",
        fields={"content_text": "轻量", "raw_payload": {"secret": True}},
        availability={"content_text": "present"},
        retrieval_context={"query": "短裤", "access_token": "secret", "raw_payload": {"secret": True}},
    )
...
def test_contract_minimum_sample_and_author_targets_stop_selection():
    ...
    result = select_candidates(
        ...
        author_cap=1, minimum_samples=2, minimum_independent_authors=2,
    )
    assert [item.canonical_source_id for item in result.decisions if item.selected] == ["note_a", "note_b"]
...
def test_query_coverage_selects_a_later_uncovered_group_after_global_target():
    ...
    candidates = [
        {"canonical_id": "group-one-a", "query_group_id": groups[0].id, "relevance": 4, "author_id": "a"},
        {"canonical_id": "group-one-b", "query_group_id": groups[0].id, "relevance": 3, "author_id": "b"},
        {"canonical_id": "group-one-surplus", "query_group_id": groups[0].id, "relevance": 2, "author_id": "c"},
        {"canonical_id": "group-two-a", "query_group_id": groups[1].id, "relevance": 1, "author_id": "d"},
    ]
    ...
    surplus = next(item for item in result.decisions if item.canonical_source_id == "group-one-surplus")
...
def test_query_coverage_selection_is_stable_when_candidate_input_order_changes():
    ...
    candidates = [
        {"canonical_id": "group-one-a", "query_group_id": groups[0].id, "relevance": 2, "author_id": "a"},
        {"canonical_id": "group-one-b", "query_group_id": groups[0].id, "relevance": 1, "author_id": "b"},
        {"canonical_id": "group-two", "query_group_id": groups[1].id, "relevance": 1, "author_id": "c"},
    ]

    first = select_candidates(groups=groups, candidates=candidates, author_cap=1, minimum_samples=2, minimum_independent_authors=2)
    second = select_candidates(groups=groups, candidates=list(reversed(candidates)), author_cap=1, minimum_samples=2, minimum_independent_authors=2)
...
def test_incomplete_when_nonempty_candidates_do_not_meet_frozen_sample_and_author_minima():
    groups = compile_query_groups(direction_id="product_marketing", subject="短裤", questions=["卖点"], competitors=[])
    result = select_candidates(groups=groups, candidates=[{"canonical_id": "note_a", "query_group_id": groups[0].id, "author_id": "a"}], author_cap=2, minimum_samples=2, minimum_independent_authors=2, detail_fetch_cap=3)
...
def test_detail_fetch_cap_is_deterministic_and_visible_as_an_exclusion_reason():
    groups = compile_query_groups(direction_id="product_marketing", subject="短裤", questions=["卖点"], competitors=[])
    candidates = [
        {"canonical_id": "note_c", "query_group_id": groups[0].id, "relevance": 1, "author_id": "c"},
        {"canonical_id": "note_a", "query_group_id": groups[0].id, "relevance": 3, "author_id": "a"},
        {"canonical_id": "note_b", "query_group_id": groups[0].id, "relevance": 2, "author_id": "b"},
    ]
    result = select_candidates(groups=groups, candidates=candidates, author_cap=1, minimum_samples=3, minimum_independent_authors=3, detail_fetch_cap=2)
