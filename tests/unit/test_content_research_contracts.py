...
def test_default_snapshot_has_one_contract_for_each_registered_direction():
    ...
    assert snapshot.effective_policy["provider_capabilities"]["xiaohongshu"]["collect_note_detail"]["status"] == "unavailable"
    assert all(item.detail_fetch_cap >= item.minimum_samples for item in policies)
    assert all((item.comment_limit, item.comment_top_level_only, item.comment_reply_depth_limit) == (30, True, 0) for item in policies)
    assert all(item.required_note_fields != ("source_id", "source_url", "body") for item in contracts)
    assert {"case_level", "repeated_observation", "provisional", "insufficient_evidence"} == set(CLAIM_EVIDENCE_STATES)
    ...
    assert snapshot.validation_result["directions"]["ugc_community"]["missing_blocking_comment_fields"] == []
...
def test_snapshot_freezes_template_only_direction_set_without_registry_inference():
    ...
    assert {item.direction_id for item in contracts} == set(snapshot.effective_policy["direction_ids"])
    assert {item.direction_id for item in policies} == set(snapshot.effective_policy["direction_ids"])
...
def test_sample_policy_rejects_detail_cap_below_minimum_samples():
    with pytest.raises(ValueError, match="detail_fetch_cap"):
        SamplePolicy(id="sp_1", schema_version="v1", direction_id="product_marketing", minimum_samples=3, minimum_independent_authors=2, author_cap=2, metadata={"detail_fetch_cap": 2})


def test_sample_policy_freezes_and_validates_comment_collection_policy():
    policy = SamplePolicy(
        id="sp_comments", schema_version="v1", direction_id="ugc_community",
        minimum_samples=30, minimum_independent_authors=5, author_cap=3,
        metadata={"detail_fetch_cap": 30, "comment_limit": 12, "comment_top_level_only": True, "comment_reply_depth_limit": 0},
    )

    assert (policy.comment_limit, policy.comment_top_level_only, policy.comment_reply_depth_limit) == (12, True, 0)
    with pytest.raises(ValueError, match="comment_limit"):
        SamplePolicy(id="sp_bad_limit", schema_version="v1", direction_id="ugc_community", minimum_samples=30, minimum_independent_authors=5, author_cap=3, metadata={"detail_fetch_cap": 30, "comment_limit": 0})
    with pytest.raises(ValueError, match="top-level"):
        SamplePolicy(id="sp_bad_depth", schema_version="v1", direction_id="ugc_community", minimum_samples=30, minimum_independent_authors=5, author_cap=3, metadata={"detail_fetch_cap": 30, "comment_top_level_only": True, "comment_reply_depth_limit": 1})
...
def test_capability_preflight_requires_all_blocking_note_and_comment_fields():
    full_capabilities = {
        "xiaohongshu": {
            "collect_note_detail": {
                "status": "supported",
                "fields": ["title", "content_text", "tags", "note_type", "metrics", "metrics_observed_at", "source_published_at", "ip_location", "media", "author"],
            },
            "collect_comments": {
                "status": "supported",
                "fields": ["comment_text", "source_published_at", "like_count", "reply_depth", "parent_note_id", "author"],
            },
        }
    }
    snapshot, _policies, contracts = build_default_snapshot(snapshot_id="rps_full", workflow_run_id="run", brief_id="rb", plan_id="rp", provider_capabilities=full_capabilities)
    preflight = evaluate_capability_preflight(contracts=contracts, provider_capabilities=full_capabilities)
    ...
    missing_comment = evaluate_capability_preflight(
        contracts=contracts,
        provider_capabilities={"xiaohongshu": {**full_capabilities["xiaohongshu"], "collect_comments": {"status": "supported", "fields": ["comment_text"]}}},
    )
...
def test_adapter_capabilities_are_frozen_once_into_snapshot_shape():
    class Adapter:
        def capabilities(self):
            return (ProviderCapability("collect_note_detail", "supported", ("title", "content_text"), {"max_limit": 1}),)
