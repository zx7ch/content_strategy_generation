...
### 2026-08-05 Task 5 营销结论 Trace 与真实验收记录

- 安全 Trace 已增加唯一的新公开 stage `marketing_conclusion`。投影逐字段构造，只公开三轨状态、满足门槛解释所需的支持笔记／独立作者计数、稳定原因码、唯一模型恢复动作，以及明确存在时的 packet-only replay 零增量；候选数、结论正文、query、prompt、quote、policy hash、note／author ID 和 provider payload 均不透出。Creator 将其显示为“营销结论判定”，并把 `analysis_unavailable` 复用到既有模型配置恢复卡，不生成第二个恢复动作。
- TDD 证据：RED 命令 `pytest -q tests/unit/test_content_research_trace_service.py tests/e2e/test_content_research_trace_api.py -k 'marketing_conclusion'` 在实现前为 `4 failed, 26 deselected`，失败原因均为公开 Trace 尚无该 checkpoint；实现后同命令为 `4 passed, 26 deselected`。正式 workflow 的成功／模型不可用两条定向场景为 `2 passed, 6 deselected`。
- 前端回归：`npm --prefix frontend test && npm --prefix frontend run build` 为 `55 passed` 且 production build 成功；保留既有 `<img>` 与 React Hook dependency warning。Task 5 涉及的 Python 文件定向 Ruff 为 `All checks passed!`。全范围 Ruff 命令仍报告 110 个既有错误（103 个可自动修复），分布在本任务外文件，因此不将全范围 lint 声明为通过。
- 规定的 Python 全回归在最终工作树上为 `5 failed, 167 passed in 453.57s`：剩余失败是一个既有 running-parent resume 断言、三个仍直接等待旧 Brief 卡而未处理结构确认的 Creator catalog 参数场景，以及一个仍按旧模型恢复后页面状态编写的 Creator 场景；本记录不把整套回归声明为通过。营销结论安全投影的定向用例和两条正式 workflow 场景均通过。
- 真实 Creator 验收 run `run_4972d58923764cef98b4cfcac381c7f1`：输入 `夏季凉感T恤`，显式确认 `T恤｜凉感｜夏季`，仅选 `product_marketing`，主目标为 `content_seeding`。系统默认 OpenAI `gpt-4o-mini` 调用成功，`llm_recovery.required=false`；完成 32 个 provider operation（2 discover + 30 detail）并持久化 30 个 packet，相对运行前增量分别为 `+32`、`+30`，无外部依赖不可用。需求／价值／表达三轨均为 `insufficient_evidence`，原因均为 `conclusion_no_qualified_candidate`；三轨 supporting note、independent author 与 extra-qualified 均为 0。报告因此正确发布为 `evidence_only_report`，audit 原因为 `insufficient_admitted_evidence`，0 citation，Creator 刷新后显示三轨中文状态与原因。该 run 是“配置完成但证据不足”的真实退化验收，不是内容成功样本，也不据此声明 Gate 4B 完成。
- 当前公开投影已经覆盖 packet-only replay 的安全字段与零增量测试，但实际 replay service 尚不会回写 `replayed_from_persisted_packets` 及两个 delta 到既有营销结论 checkpoint；该生产端位于本 Task 5 获批文件范围之外。因此本次只声明投影合同与人工持久化 checkpoint 的 API 回归通过，不声明真实 replay Trace 标记已端到端完成。
