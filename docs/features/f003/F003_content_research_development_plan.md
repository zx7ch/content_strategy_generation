...
- Pre-research 必须把不可预测的用户输入生成为受 schema 约束的 `subject_structure`：`canonical_subject`、`subject_type`、`core_entities`、`research_intents`、`context_modifiers` 与 `synonym_groups`。该结构来自 Pre-research LLM，不依赖固定业务品类词表；后端执行规范化、数量/长度限制、空核心对象拒绝与结构一致性校验。Brief 以“核心对象｜意图｜场景”紧凑行供用户确认或修改。
- 确认 Brief 时必须为每个已请求方向把已确认 `subject_structure`、由其确定性编译的 `subject_anchors`、核心对象 anchor groups、冻结同义词、matching mode、claim-type 可引用字段和全部 `query_group_ids` 同时冻结到 `RunPolicySnapshot` 与对应 `DirectionContract`；两份 payload 必须完全一致，并记录 Provider、模型、Prompt/Schema 版本与输入指纹。不得把包含方向问题的完整 query 当作字面匹配词，也不得在 admission 阶段重新调用 LLM 解释主题。
...
- 正式主体相关性采用双层 admission gate：source packet 必须保存命中冻结 `QueryGroup` 的谱系，但该谱系只是必要条件；candidate 的直接 quote 还必须按冻结 matching mode 命中核心对象 anchor 或其冻结同义词，并来自该 claim type 允许的字段。`research_intents` 与 `context_modifiers` 只用于限定查询和解释范围，不能脱离核心对象单独放行。任一条件不满足即以稳定 reason code `query_subject_not_supported` rejected，互动指标不能覆盖该拒绝。`product_marketing.message_angle` 继续允许 title 或正文直接支撑，但 quote 本身仍须命中核心对象。
