...
stateDiagram-v2
    ...
    created --> running: start_run\nrun 已创建，至少有首个 pending step

    running --> pausing: pause_run\n当前 run 可暂停
    pausing --> paused: pause_ack\n当前 step 到达安全边界
    paused --> running: resume_run\nrun 未 cancelled / failed / succeeded

    running --> cancelling: cancel_run\n当前 run 可取消
    pausing --> cancelling: cancel_run\n暂停过程中用户取消
    paused --> cancelling: cancel_run\n暂停后用户取消
    cancelling --> cancelled: cancel_ack\nactive step/job 已终止或被 commit guard 拦截

    running --> waiting_user: require_user_input\nstep 明确需要人工确认
    waiting_user --> running: user_continue\n用户提交确认或编辑

    running --> succeeded: complete_run\n所有 required steps 成功
    running --> failed: fail_run\n不可恢复错误或重试耗尽
    pausing --> failed: fail_run\n暂停途中发生不可恢复错误
...
stateDiagram-v2
    ...
    retrying --> running: start_step

    running --> succeeded: complete_step
    running --> retrying: retry_step
    running --> failed: fail_step
    running --> cancelled: cancel_step
    ...
    note right of retrying
        可恢复错误后的等待状态
        到达 retry 时间后重新 running
    end note

    note right of running
        complete_step 需要：
        run 未 cancelling/cancelled
        且 commit guard 通过
    end note
...
stateDiagram-v2
    ...
    queued --> running: Worker lease
    retrying --> running: Worker lease

    running --> succeeded: ack success
    running --> retrying: retryable error / lease expired
    running --> failed: permanent error / retry exhausted
    ...
    retrying --> cancelled: cancel_run
    paused --> cancelled: cancel_run
    running --> cancelled: cancel_run / lease cancel
    ...
    note right of queued
        等待 Worker 获取 lease
        pause 后恢复也会回到 queued
    end note

    note right of running
        Worker 正在执行
        需要 lease 保护
        ack success 后进入 succeeded
    end note

    note right of retrying
        retryable error
        或 lease expired 后仍可重试
        到达 retry 时间后重新被 lease
    end note

    note right of paused
        run 暂停时
        queued / retrying job 可进入 paused
    end note
```
...
其中 `WorkflowStep Executor` 的执行分层必须保持清晰：

```text
StepExecutorRegistry：负责“找谁执行”
StepExecutor：负责“怎么按 workflow 规则执行”
Agent runner：负责“业务本身怎么做”
WorkflowRunManager：负责“状态和 artifact 怎么落库”
```

也就是说，`step_name` 到执行能力的映射由 `StepExecutorRegistry` 完成；`StepExecutor` 负责构建 `StepContext`、调用 runner、处理 child task / artifact refs 等 workflow 外壳；真实 Strategy / Generation / Tool 能力沉到 Agent runner；所有 run / step / child task / artifact / event 的状态转换仍统一收口到 `WorkflowRunManager`。
...
payload_mode = patch。
payload_json = 相对 parent artifact 的增量修改。
不覆盖原 note。
```

Revision 的关键原则是：**逻辑上保留一个新 artifact version，物理上不要求复制完整 payload**。

也就是说，revision/rewrite 仍然必须写入一条新的 `workflow_artifacts` 记录，用 `artifact_id / parent_artifact_id / artifact_version / status` 表达版本事实；但该记录的 `payload_json` 默认只保存增量 patch，避免每次微调都复制完整笔记正文、策略或聚合结果。

#### Payload Mode

artifact payload 分为两种模式：

```text
snapshot
= payload_json 是完整可渲染内容。

patch
= payload_json 是相对 parent_artifact_id 的增量修改。
```
...
- `snapshot` 用于初始产物、最终结果、被用户接受的产物，以及需要直接发布或长期稳定读取的产物。
- `patch` 用于 revision/rewrite，默认只记录相对父版本变化的字段或操作。
- patch 不决定哪个版本是当前推荐版本；推荐、发布和隐藏旧版本由 artifact `status`、版本策略和 review/accept 规则决定。
- 同一 parent 可以有多个 sibling rewrite，每个 rewrite 都是独立 artifact，不能互相覆盖。
- finalization 或用户 accept 时，应把最终选择 materialize 成 `snapshot`，避免 publish candidate 读取时沿长链回放。

#### Patch Materialization

对外读取 artifact 时，API 不应把 patch 细节泄漏给前端。

```text
读取 artifact
-> 如果 payload_mode = snapshot，直接返回 payload_json
-> 如果 payload_mode = patch，沿 parent_artifact_id 读取父版本
-> materialize 父版本
-> 应用 patch
-> 返回完整可渲染 payload
```

materialization 必须有保护：

- parent artifact 缺失时返回清晰错误或可诊断 payload，不能静默生成错误结果。
- patch 链有最大深度，超过阈值应停止并报错。
- 检测循环引用，禁止无限递归。
- patch 应声明 `base_artifact_id/base_artifact_version`，用于发现基线不匹配和并发冲突。

Message Timeline 和 `/threads/{thread_id}/result` 可以引用 patch artifact，但返回给前端的 artifact card/result 必须是 materialized 完整内容。
