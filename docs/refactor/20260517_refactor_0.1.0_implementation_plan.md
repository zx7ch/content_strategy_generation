...
flowchart TD
    ...
    T1011["T10.1.1 WorkflowRun 执行分发与前端进度卡片"]
    T102["T10.2 Compare Versions 用户入口与按钮"]

    T1 --> T2 --> T21 --> T3 --> T4 --> T5 --> T6 --> T61 --> T7 --> T71 --> T72 --> T8 --> T81 --> T82 --> T83 --> T9 --> T91 --> T10 --> T101 --> T1011 --> T102
```
...
- 状态：Done
- 完成日期：2026-05-24
- 开发指南：[instructions/T9_guide.md](../../instructions/T9_guide.md)
- 交付代码：
  - `frontend/src/app/creator/page.tsx`
  - `frontend/src/lib/api.ts`
- 交付测试：
  ...
  - `tests/e2e/test_creator_message_workflow_v2.py`
- 验证结果：
  - `npm run lint`（frontend）：passed（存在非阻塞 warning）
  - `npm run build`（frontend）：passed
  - `pytest -q tests/e2e/test_creator_thread_api.py tests/e2e/test_creator_message_workflow_v2.py`：15 passed
- 说明：已将 Creator 页面协议切到 workflow-v2：页面加载读取 `/threads/{thread_id}/timeline` 和 active run snapshot；发送消息只走 `POST /threads/{thread_id}/messages` 并使用 `active_run_snapshot` 更新任务状态；SSE 改订阅 `/workflow-runs/{run_id}/events`；artifact result message 可用 hydrated artifact refs 渲染结果卡片；已删除前端关键词 `inferTaskIntent` 启动 workflow 逻辑，不再从 `active_job_id`、旧 session events 或本地 task state 推断 workflow-v2 业务状态。旧 job/session API helper 保留在 API 层作为兼容，不参与 Creator 页面新状态路径；已补齐 ESLint 配置与 Next 14 兼容的 lint 依赖。
...
- 状态：Done
- 完成日期：2026-05-24
- 开发指南：[instructions/T9.1_guide.md](../../instructions/T9.1_guide.md)
- 交付代码：
  - `frontend/src/app/creator/page.tsx`
  - `frontend/src/lib/api.ts`
- 交付测试：
...
- 验证结果：
  - `npm run lint`（frontend）：passed（存在非阻塞 warning）
  - `npm run build`（frontend）：passed
  - `pytest -q tests/e2e/test_thread_timeline_artifacts.py`：4 passed
- 说明：已在 Creator artifact result card 中展示 artifact version metadata，包括当前版本、状态、payload mode、parent id；带 `parent_artifact_id` 的 revision/patch artifact 会展示可展开版本链，能看到原版与 revision 版本。前端显示内容优先使用 hydrated/materialized payload，因此 patch artifact 对用户展示为完整笔记内容，不暴露 patch operations。T10 的完整端到端验收与旧链路清理前置条件已满足。
...
- 状态：Done
- 完成日期：2026-05-24
- 开发指南：[instructions/T10_guide.md](../../instructions/T10_guide.md)
- 交付代码：
  - `app/api/routes/router.py`
  - `app/models/schemas.py`
  - `frontend/src/lib/api.ts`
...
- 交付测试：
  - `tests/acceptance/test_creator_workflow_v2_full_loop.py`
  - `tests/e2e/test_creator_message_intent_api.py`
  - `tests/e2e/test_job_control_api.py`
- 验证结果：
  - `pytest -q tests/acceptance/test_creator_workflow_v2_full_loop.py`：3 passed
  - `pytest -q tests/e2e/test_creator_message_workflow_v2.py tests/e2e/test_creator_message_revision_workflow.py tests/e2e/test_creator_message_rerun_workflow.py tests/e2e/test_thread_timeline_artifacts.py`：12 passed
  - `pytest -q tests/e2e/test_creator_message_intent_api.py tests/e2e/test_job_control_api.py`：10 passed
  - `cd frontend && npm run lint`：passed（保留既有 React hook dependency warnings）
  - `cd frontend && npm run build`：passed（保留同上 warnings）
- 说明：已完成 T10 端到端验收与旧链路清理：`POST /threads/{thread_id}/workflow` 已降级为 workflow-v2 兼容转发入口，内部走 `ConversationOrchestrator` / `WorkflowRunManager.start_run` / canonical steps，不再为新任务创建旧 session/job 真相；响应补充 `run_id`、`command_result`、`active_run_snapshot`、`compatibility_mode`，前端类型同步。新增 acceptance 覆盖 workflow-v2 全链路：创建 Thread、message 启动 run、constraint、pause/resume/cancel/status 相关状态、workflow SSE replay、snapshot 恢复、revision、regenerate、rerun、新 run 完成后 publish candidates、artifact_result timeline 与版本链。README 已补充自然语言意图只走 message endpoint、snapshot/timeline/workflow-events 恢复和 legacy workflow endpoint 兼容行为。旧 job control API 仍保留给历史 session/job 路径，相关 e2e 改为显式构造 legacy session/job fixture，不再依赖 `/threads/{id}/workflow` 创建旧 job。
  - [x] 新创建任务和前端状态恢复不依赖 `active_workflow_session_id`、`active_job_id` 或 session events fallback。
  - [-] 旧 session/job 表删除、历史数据迁移、job control API 收敛不属于 T10，后续应作为 legacy deprecation/control API convergence 独立任务处理。
  - [-] 真实 LLM/Spider/RAG 的恢复和外部调用安全边界仍归入 T10.1。

### Bugfix 记录

- 2026-05-24：修正 legacy `/threads/{id}/workflow` 入口不应继续创建旧 session/job 的架构漂移，改为 workflow-v2 兼容转发；同步修正旧 job/control e2e，使其显式构造 legacy fixture，避免继续把兼容入口当成旧任务真相。
...
- 状态：Done
- 完成日期：2026-05-24
- 开发指南：[instructions/T10.1_guide.md](../../instructions/T10.1_guide.md)
- 交付代码：
  - `app/services/conversation_orchestrator.py`
  - `app/services/step_executors.py`
  - `app/workers/job_worker.py`
- 交付测试：
  ...
  - `tests/unit/test_conversation_orchestrator.py`
- 验证结果：
  - `pytest -q tests/integration/test_workflow_step_recovery_e2e.py tests/unit/test_conversation_orchestrator.py`：15 passed
  - `pytest -q tests/integration/test_workflow_pause_cancel_jobs.py tests/integration/test_workflow_job_worker.py`：11 passed
  - `ACCEPTANCE_RUN_REAL=1 python3 -m pytest -q -m real_dependency tests/integration/test_workflow_real_step_boundaries.py`：3 passed
- 说明：已完成 T10.1 的代码侧 hardening 与真实依赖验收：`LLMStructuredConstraintClassifier` 已接入真实 LLM client 的 JSON-only structured classification 路径；workflow step executor 在 `attach_artifact` 前检查 run status，防止真实 Spider/LLM/RAG late success 在 pause/cancel 后提交 artifact；`JobWorker` 能将 pre-commit block 转换为 pause/cancel safe-boundary ack，而不是 generic job failure；lease expired workflow job 与 WorkflowStep 的 retry/failed 同步已有 e2e 覆盖。真实依赖 gate 已在具备 `ACCEPTANCE_RUN_REAL=1`、LLM 凭据、`XHS_SPIDER_COOKIES`、RAG embedding 依赖、`PyExecJS` 的环境完成，覆盖真实 LLM structured classifier、真实 Spider smoke、真实 RAG index artifact commit。
  - [x] fake/deterministic 安全边界和 lease recovery 覆盖已通过。
  - [x] real_dependency 测试文件已就位，要求 `ACCEPTANCE_RUN_REAL=1`、LLM provider key、`XHS_SPIDER_COOKIES`、RAG embedding 依赖。
  - [x] 真实依赖门禁已执行并通过：`ACCEPTANCE_RUN_REAL=1 python3 -m pytest -q -m real_dependency tests/integration/test_workflow_real_step_boundaries.py`。

### Bugfix 记录

- 2026-05-24：补齐 executor pre-commit guard，防止外部调用 late success 在 pause/cancel 后写入 artifact；补齐 `LLMStructuredConstraintClassifier` 真实 LLM client JSON 解析路径；新增 deterministic recovery e2e 与 real_dependency gate。当时因真实依赖尚未执行先保持 Blocked，随后由真实门禁通过解除。
- 2026-05-24：补齐 real_dependency gate 的真实环境修正：RAG 测试 seed 改为通过 `ThreadStore` 创建 thread/user message，`QualityScore` 按 dataclass 转 dict；根依赖补充 `PyExecJS` 以满足 Spider 子模块 `execjs` 运行时。用户环境执行 `ACCEPTANCE_RUN_REAL=1 python3 -m pytest -q -m real_dependency tests/integration/test_workflow_real_step_boundaries.py`，结果 3 passed，T10.1 标记 Done。

## T10.1.1 WorkflowRun 执行分发与前端进度卡片
...
修复 T10/T10.1 切换到 WorkflowRun 架构后的执行闭环回归：用户通过 message endpoint 启动 run 后，后端必须自动分发并推进 workflow steps；前端必须从 workflow events/snapshot 显示具体任务执行进度，而不是只展示“已开始新的创作任务。”
...
```text
app/services/workflow_dispatcher.py
app/services/conversation_orchestrator.py
app/workers/job_worker.py
app/agents/orchestrator.py
app/memory/job_store.py
...
```text
start workflow 后 enqueue 第一个 pending step
worker 成功完成 step 后自动 enqueue 下一步或 complete run
workflow-bound job 不依赖 legacy session row
startup worker 使用生产 step executor registry
前端根据 workflow event type + snapshot steps 显示具体进度卡片
```
...
- 状态：Done
- 完成日期：2026-05-24
- 开发指南：[instructions/T10.1.1_guide.md](../../instructions/T10.1.1_guide.md)
- 追加交互契约：[docs/refactor/20260524_T10.1.1_frontend_backend_contract.md](20260524_T10.1.1_frontend_backend_contract.md)
- 交付代码：
  - `app/services/workflow_dispatcher.py`
  - `app/services/conversation_orchestrator.py`
  - `app/workers/job_worker.py`
...
- 交付测试：
  ...
  - `tests/integration/test_workflow_job_worker.py`
- 验证结果：
  - `pytest -q tests/e2e/test_creator_message_workflow_v2.py tests/integration/test_workflow_job_worker.py`：13 passed
  - `pytest -q tests/integration/test_workflow_step_recovery_e2e.py tests/unit/test_conversation_orchestrator.py`：15 passed
  - `pytest -q tests/integration/test_workflow_pause_cancel_jobs.py tests/integration/test_workflow_job_worker.py`：12 passed
  - `cd frontend && npm run lint`：passed（保留既有 React Hook dependency warnings）
  - `cd frontend && npm run build`：passed（保留既有 React Hook dependency warnings）
- 说明：已补齐 WorkflowRun 执行分发闭环。`ConversationOrchestrator` 启动 run 后通过 `WorkflowStepDispatcher` 入队第一步；`JobWorker` 成功完成 workflow step 后自动入队下一步，所有步骤成功/跳过后 complete run，并写入 artifact_result message。workflow-bound job 可在没有旧 session row 的情况下 lease/execute；startup worker 改为使用生产 canonical step executor registry。前端进度卡片现在按 snapshot steps 计算 `已完成/总数/百分比`，并在 workflow event 缺少 `payload.message` 时按 event type 映射“任务已创建 / 已拆解创作步骤 / 正在执行：...”等用户可读进展。
- 2026-05-24 追加 contract：针对手动验证暴露的两个交互缺口，补充 `docs/refactor/20260524_T10.1.1_frontend_backend_contract.md`。该 contract 明确：取消/终态 run 上的新创作请求必须启动新 run，不得走 `add_constraint` 500；后台 step 进展只能更新任务卡片，`step_failed` 不得被前端当作 whole-run failure 反复追加“任务执行失败，请重试。”；snapshot/events 订阅必须防重入、节流并在切换 run/thread 时关闭旧连接。

### Bugfix 记录

- 2026-05-24：修复 WorkflowRun run 创建后未自动执行 step 的回归；修复前端只在 `payload.message` 存在时显示进度导致 workflow-v2 SSE 看起来“无流式进度”的问题。
- 2026-05-24：修复前端将 `run_failed/step_failed` 追加成聊天气泡导致“任务执行失败，请重试”不断刷屏和页面持续下滑的问题；失败/取消现在只更新任务进度卡片，进度日志不触发聊天自动滚动，snapshot 刷新做 500ms 节流。同步修复取消中的历史对话再次输入创作需求时误走 `add_constraint` 导致 500 的问题，新的创作请求会开启新的 WorkflowRun。
...
- 交付测试：-
- 验证结果：-
- 说明：未开始实现。T9.1/T10 已完成，artifact version chain UI 与完整 workflow-v2 验收基线已稳定，可进入 compare versions 开发。
...
| T10.1.1 WorkflowRun 执行分发与前端进度卡片 | 是 | T10.1 |
| T10.2 Compare Versions 用户意图与按钮 | 是 | T9.1, T10.1.1 |
