...
**SynthesisWorker 职责**：
- 从证据簇生成目标 5 张、实际 1-5 张的候选卡片
- 对候选去同质化
- 绑定证据引用
- 在证据不足时返回零结果兜底所需的 rewrite suggestions，不返回长篇研究报告
...
| 搜索结果为空 | 先做一次 query rewrite + 单轮重搜；仍为空则返回零结果兜底 | 不展示空卡，占位区改为 2-3 个英文 rewrite suggestion chips |
| 候选不足 5 张 | 保留真实可交付候选，禁止弱填充 | 返回 1-4 张真实候选卡，不补假卡 |
...
| `SynthesisWorker` | 从证据簇生成目标 5 张、实际 1-5 张的候选卡片，去同质化，绑定引用 | 只输出轻量候选或 rewrite suggestions，不输出长报告 |
...
| `handoff` | 把已确认候选移交 strategy | 用户对 active branch 中的有效候选执行 confirm |

`Evaluator` 为 `Planner` 内部策略模块，不是独立 agent。每轮在动作选择前运行，输入：
- `ambiguity_score`
- `evidence_coverage`
- `evidence_diversity`
- `candidate_confidence`
