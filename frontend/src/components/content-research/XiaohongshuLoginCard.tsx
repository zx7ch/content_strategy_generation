...
export function XiaohongshuLoginCard() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setQrPending(false));
    poll(); const timer = window.setInterval(poll, 1500); return () => window.clearInterval(timer);
  }, [qrPending]);

  const startQr = async () => {
    setError(null);
    try { const result = await startXHSQRLogin(); setQrImage(result.qr_image_data_url ?? null); setQrPending(result.status === "pending"); }
    catch { setError("二维码暂不可用，请稍后重试或粘贴 Cookie"); }
  };
  const saveCookie = async () => {
    if (!cookie.trim()) return;
    setError(null);
    try { setStatus(await saveXHSManualCookie(cookie)); setCookie(""); }
    catch { setCookie(""); setError("Cookie 保存失败，请确认后重试"); }
  };
  ...
  return <section className="mb-4 rounded-xl border border-line bg-white p-4" aria-label="小红书登录">
    ...
    <button type="button" onClick={() => void startQr()} className="mt-3 rounded-lg bg-[#486b5b] px-3 py-1.5 text-xs font-medium text-white">扫码登录</button>
    ...
    <div className="mt-2 flex gap-2"><button type="button" onClick={() => void saveCookie()} disabled={!cookie.trim()} className="rounded-lg border border-line px-3 py-1.5 text-xs disabled:opacity-40">保存 Cookie</button>{status?.authenticated && <button type="button" onClick={() => void clear()} className="rounded-lg border border-line px-3 py-1.5 text-xs">清除</button>}</div>
